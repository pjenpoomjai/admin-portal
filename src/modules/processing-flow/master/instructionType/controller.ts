...
import { IGetMasterConfigInstructionTypeRequest } from 'shared/adapters/interfaces/masterConfig/queries'
...
import { mapMasterConfigInstructionType } from './dto'
...
export const useProcessingFlowMasterInstructionTypeController = (
	...
	const {
		...
		ModelMasterConfig.MasterConfigInstructionType,
		IGetMasterConfigInstructionTypeRequest
	>({
		adapter,
		queryKey: 'getMasterConfigInstructionType',
		openCommonDialog,
		closeCommonDialog,
		initRequest,
		transformData: mapMasterConfigInstructionType,
	})
	...
	const handleOnEdit = useCallback((_service: IInstructionType) => {}, [])

	const handleOnDuplicate = useCallback((_service: IInstructionType) => {}, [])
